...
def scrapper(url_page_list):
    ...
    sql = 'CREATE TABLE IF NOT EXISTS `xuanwu4` (`Category`  NVARCHAR(256),`Names` NVARCHAR(100),`Link` NVARCHAR(256), `Times` DATE, \
                                                PRIMARY KEY (Names))'
...
##            Link = cells[2].text
            ...
            sql = 'REPLACE INTO xuanwu4(`Category` ,`Names` ,`Link`,`Times` )\
                   VALUES\
                  (%s, %s, %s, %s);'

            cursor.execute(sql,
                           (Category,Name,Link,Time))
