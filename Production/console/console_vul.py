...
def write_html(filename,report_id,request_id):
    ...
    #print(text)
    #with open("http://cti-pub-files.oss-cn-shanghai.aliyuncs.com/pages/patch.html", 'r') as f1:
        #list1 = f1.readlines()

    #with open("http://cti-pub-files.oss-cn-shanghai.aliyuncs.com/pages/high_level.html", 'r') as f2:
        #list2 = f2.readlines()
    # response = requests.get('http://cti-pub-files.oss-cn-shanghai.aliyuncs.com/pages/patch.csv')
    # html = response.text
    # list1 = re.findall(r'.*?\n', html)
    ...
    # response2 = requests.get('http://cti-pub-files.oss-cn-shanghai.aliyuncs.com/pages/high_level.csv')
    # html2 = response2.text
    # list2 = re.findall(r'.*?\n', html2)
    list1 = csv.reader(open('high_level.csv', 'r'))
    list2 = csv.reader(open('patch_ch.csv', 'r'))

    #response3 = request.get('http://cti-pub-files.oss-cn-shanghai.aliyuncs.com/pages/high_level.html')
    ...
    with open(filename, 'w') as f:
        ...
        f.write('''    <title>云雀网络安全漏洞威胁情报</title>\n''')
        ...
        f.write('''            <div class="eyscrollogo"><a href="#" class="logo"><img src="https://cti-pub-files.oss-cn-shanghai.aliyuncs.com/skylark.png" alt="EY Ernst Young logo" border="0"></a></div>\n''')
        ...
        f.write('''                        <h1 class="eyhero-headline-1">云雀网络安全漏洞威胁情报<br>感知、抵御、应对</h1>\n''')
        days1 = datetime.date(2018, 07, 01)
        days2 = datetime.date.today()
        num = (days2-days1).days
        weeks = (num//7)+1
        f.write('''                        <h2 class="eyhero-subheading-1">云雀网络安全漏洞威胁情报(第%s周)</h2> </div>\n'''%weeks)
        f.write('''                </div>\n''')
        ...
        #f.write('''<p>%s</p>''',list3[0])
        #for list_each3 in list3[:4]:
            #f.write('''<p>%s</p>''' % list_each3)

        f.write('''<p>%s</p>''' % text1)

        f.write('''<iframe src="https://cti-pub-files.oss-cn-shanghai.aliyuncs.com/%s/pie.html" width=800px  height=380px  frameborder="0"  scrolling="no"></iframe><h3>High risk vulnerability</h3>''' % report_id)
        ...
        #for list_each3 in list3[5:]:
            #f.write('''<p>%s</p>''' % list_each3)
        #f.write('''<p>%s</p>''' % last_txt)

        f.write('''<p>%s</p>''' % text2)


        f.write('''<iframe src="https://cti-pub-files.oss-cn-shanghai.aliyuncs.com/%s/bar1.html" width="750" height="420" scrolling="no" frameborder="0"></iframe>'''%report_id)
        #f.write('''<p>%s</p>''' % list3[1])
        f.write('''   ''')
        f.write('''   <!-- <div class="col large-3">  </div> --></div></div></section><section class="section4" id="section4" data-section-title="漏洞补丁" style="background-color:#F0F0F0"><div class="container"><div class="row"><div class="col large-9"><h3>漏洞补丁</h3> </div></div><div class="row"><div class="col large-9">''')
        # f.write('''<table border="1" class="dataframe">''')
        # f.write('''<thead>''')
        # f.write('''<tr style="text-align: right;">''')
        # f.write('''<th id=thc1>cveId</th>''')
        ...
        f.write('''     <!-- <div class="col large-3">  </div> --></div></div></section><section class="section4" id="section4" data-section-title="漏洞补丁" style="background-color:#F0F0F0"><div class="container"><div class="row"><div class="col large-9"><h3>当月每天漏洞数量列示</h3> </div></div><div class="row"><div class="col large-9">''')
        f.write('''<iframe src="https://cti-pub-files.oss-cn-shanghai.aliyuncs.com/%s/bar2.html" width="750" height="370" scrolling="no" frameborder="0"></iframe>'''% report_id)
        #f.write('''<p>%s</p>''' % list3[2])
        ...
        f.write('''\n''')
        f.write('''</html>\n''')
    with open('report-v15.html', 'rb') as fileobj:
        directory = report_id+'/report-v15.html'
        result = bucket.put_object(directory,fileobj)
def main():
    write_html('vul_test3.html','测试机','测试机')
