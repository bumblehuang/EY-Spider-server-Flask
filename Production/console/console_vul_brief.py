...
def write_html(filename,report_id,request_id):
    ...
    list2 = csv.reader(open('patch_ch_small.csv', 'r'))
    ...
    with open(filename, 'w') as f:
        ...
        f.write('''    <title>云雀网络安全漏洞威胁情报%s</title>\n'''%postfix)
...
\n''')
        ...
        f.write('''<p class="topdetails">若该邮件无法正常显示，请<a href="https://cti-pub-files.oss-cn-shanghai.aliyuncs.com/%s/report-v15.html" target="_blank">点击此处查看详情</a></p>\n'''%report_id)
        ...
        f.write('''                    <div class="container">\n''')
        f.write('''                        <div class="maincolumn">\n''')
        f.write('''                            <h4>更多详细内容请<a href="https://cti-pub-files.oss-cn-shanghai.aliyuncs.com/%s/report-v15.html" target="_blank">点击此处查看</a></h4></div>\n'''%report_id)
        f.write('''                    </div>\n''')
        f.write('''                </div>\n''')
        f.write('''    </article>\n''')
        ...
        f.write('''            <p class="detail">声明：以上内容均来自互联网，由CTI网络威胁情报团队整理，其版权归属原作者所有，其观点并不代表我方观点和意见，请酌情参考或采用。若对以上所引用内容有任何疑问或问题，请与services@eycyber联系。</p>\n''')
        f.write('''        </div>\n''')
        f.write('''    </footer>\n''')
        f.write('''    </body>\n''')
        f.write('''\n''')
        f.write('''</html>\n''')
    with open('test_vul_word0705.html', 'rb') as fileobj:
        directory = report_id+'/test_vul_word0705.html' 
...
def main():
    ...
    write_html('test_vul_word0705.html','测试机','测试机')
